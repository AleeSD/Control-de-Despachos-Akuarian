...
import { ESTADOS, colorDeEstado, ESTADOS_CON_NOTA } from "@/lib/estados";
...
// De los estados con nota, los que exigen nota no vacía.
const NOTA_OBLIGATORIA = new Set(["otro"]);
...
function placeholderNota(estado: string): string {
  switch (estado) {
    case "cancelado":         return "Motivo de cancelación (opcional)…";
    case "no salió a ruta":   return "Motivo por el que no salió (opcional)…";
    case "cliente no estaba": return "Observación adicional (opcional)…";
    case "reprogramado":      return "Motivo de reprogramación (opcional)…";
    default:                  return "Describe el motivo (obligatorio)…";
  }
}
...
export default function PedidosTable({ pedidos, editable }: Props) {
  ...
  // Mientras el popover está abierto, muestra el estado nuevo (aún no guardado)
  // en el select para que el usuario vea lo que está configurando.
  function resolverEstado(p: PedidoVista): string {
    if (popover?.clave === p.clave_pedido) return popover.estadoNuevo;
    return estadoOv[p.clave_pedido] ?? p.estado;
  }

  function seleccionarEstado(clave: string, estadoNuevo: string) {
    const estadoAnterior =
      estadoOv[clave] ??
      pedidos.find((p) => p.clave_pedido === clave)?.estado ??
      "pendiente";

    if (ESTADOS_CON_NOTA.has(estadoNuevo)) {
      setPopover({ clave, estadoNuevo, estadoAnterior, nota: "", fecha: "" });
    } else {
      // Estado limpio: guarda de inmediato y limpia nota_estado.
      setEstadoOv((o) => ({ ...o, [clave]: estadoNuevo }));
      startTransition(() => {
        void actualizarEstado(clave, estadoNuevo, { nota: null });
      });
    }
  }

  function guardarPopover() {
    if (!popover) return;
    const { clave, estadoNuevo, nota, fecha } = popover;
    setEstadoOv((o) => ({ ...o, [clave]: estadoNuevo }));
    setPopover(null);
    startTransition(() => {
      void actualizarEstado(clave, estadoNuevo, {
        nota: nota.trim() || null,
        fechaReprogramada: fecha || null,
      });
    });
  }

  function cancelarPopover() {
    setPopover(null);
  }

  const guardarDisabled =
    !popover ||
    (NOTA_OBLIGATORIA.has(popover.estadoNuevo) && !popover.nota.trim()) ||
    (popover.estadoNuevo === "reprogramado" && !popover.fecha);
  ...
  return (
    <>
      {/* Overlay / popover de motivo y fecha */}
      {popover && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div
            className="absolute inset-0 bg-on-surface/20"
            onClick={cancelarPopover}
          />
          <div className="relative z-10 bg-surface-container-lowest rounded-xl shadow-soft border border-surface-variant p-lg w-80 space-y-md">
            <h4 className="text-title-lg font-semibold text-on-surface capitalize">
              {popover.estadoNuevo}
            </h4>

            <div>
              <label className="block text-label-md text-on-surface-variant mb-sm">
                Motivo{NOTA_OBLIGATORIA.has(popover.estadoNuevo) ? " *" : " (opcional)"}
              </label>
              <textarea
                autoFocus
                rows={3}
                value={popover.nota}
                onChange={(e) =>
                  setPopover((prev) => prev ? { ...prev, nota: e.target.value } : null)
                }
                placeholder={placeholderNota(popover.estadoNuevo)}
                className="w-full px-3 py-2 border border-outline-variant rounded-input text-body-md text-on-surface focus:outline-none focus:ring-1 focus:ring-primary-container resize-none"
              />
            </div>

            {popover.estadoNuevo === "reprogramado" && (
              <div>
                <label className="block text-label-md text-on-surface-variant mb-sm">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPopover((prev) => prev ? { ...prev, fecha: e.target.value } : null)
                  }
                  className="w-full px-3 py-2 border border-outline-variant rounded-input text-body-md text-on-surface focus:outline-none focus:ring-1 focus:ring-primary-container"
                />
              </div>
            )}
            ...
            <div className="flex gap-sm justify-end pt-sm">
              <button
                onClick={cancelarPopover}
                className="px-4 py-2 text-body-md text-on-surface-variant hover:text-on-surface border border-outline-variant rounded-input transition-colors"
              >
                Cancelar
              </button>
              <button
                onClick={guardarPopover}
                disabled={guardarDisabled}
                className="px-4 py-2 text-body-md text-on-primary bg-primary rounded-input disabled:opacity-40 disabled:cursor-not-allowed hover:bg-on-primary-fixed-variant transition-colors"
              >
                Guardar
              </button>
            </div>
                    ...
                    <tr className="hover:bg-surface-bright transition-colors group">
                      ...
                      <td className="px-md py-sm">
                        {editable ? (
                          <select
                            value={estado}
                            onChange={(e) => seleccionarEstado(p.clave_pedido, e.target.value)}
                            title={p.nota_estado ?? undefined}
                          ...
                          <span title={p.nota_estado ?? undefined}>
                        ...
                        <td colSpan={11} className="px-md py-md">
                          <div className="rounded-lg border border-surface-variant bg-surface-container-lowest p-md">
                            <div className="flex flex-wrap gap-x-xl gap-y-1 mb-md text-body-md">
                              ...
                              {p.nota_estado && (
                                <span><b>Motivo:</b> {p.nota_estado}</span>
                              )}
                              {p.fecha_reprogramada && (
                                <span><b>Reprogramado para:</b> {fechaCorta(p.fecha_reprogramada)}</span>
                              )}
                            </div>
